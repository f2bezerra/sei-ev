...
function getFrameDocument(frame) {
	...
	if (typeof frame == "string") {
		switch (frame) {
			...
			case "visualizacao":
			case "visualizador":
				frame = window.top.document.getElementById("ifrVisualizacao") || window.top.document.getElementById("ifrConteudoVisualizacao");
...
async function getHistorico(filter) {
	...
	if (!urlHistorico) urlHistorico = (m = $(docArvore.body).html().match(/controlador\.php\?acao=procedimento_consultar_historico[^'"]+/i)) && m[0].replace(/&amp;/g, "&");
...
function addCommand(id, icon, title, list, callback) {
	...
	var visualizador = getFrameDocument('visualizador');
	var btn = document.getElementById(id);
	if (!btn && visualizador) btn = visualizador.getElementById(id);

	if (!btn) {
		var div_commands = document.getElementById("divComandos");
		if (!div_commands && visualizador) div_commands = visualizador.getElementById("divArvoreAcoes");
