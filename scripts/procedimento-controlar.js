...
async function createTableFilter(table, filterSettings) {
	...
	if (!tableFilter) {
		...
		filtros.push({ id: "filtro", key: "filtro", text: "Nenhum", value: "" });
		filtros.push("-");

		let pontosControle = await PontoControle.list();
		pontosControle.forEach(ponto => filtros.push({ id: "filtro", key: "filtro", text: ponto }));
